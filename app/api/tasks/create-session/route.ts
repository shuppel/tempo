...
function validateAllTasksIncluded(
  ...
  const missingTasks = Array.from(originalTaskMap.values()).map(
    (task) => task.title,
  );
    ...
    for (let i = 0; i < block.timeBoxes.length; i++) {
      ...
      if (timeBox.type === "work") {
        ...
        if (
          ...
          (i === block.timeBoxes.length - 1 ||
            block.timeBoxes[i + 1].type === "work")
        ) {
          console.log(
...
async function retryWithBackoff<T>(
  ...
  try {
    ...
    if (
      ...
      console.log(
        `API overloaded, retrying in ${delay}ms (attempt ${retryCount + 1})`,
      );
        ...
        try {
          ...
          const calculatedDuration = parsedData.storyBlocks.reduce(
            (acc: number, block: StoryBlock) =>
              acc + calculateTotalDuration(block.timeBoxes),
            0,
          );
