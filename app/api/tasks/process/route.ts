...
// Define types for the parsed data structure
  ...
  "stories": [
    {
      ...
      try {
        ...
        processedData = {
          stories: parsedData.stories.map((story: ParsedStory) => ({
            ...story,
        ...
        processedData.stories.forEach((story: APIProcessedStory) => {
          story.tasks = story.tasks.map((task: APIProcessedTask) => {
        ...
        processedData.stories.forEach((story: APIProcessedStory) => {
          story.tasks.forEach((task: APIProcessedTask) => {
        ...
        const processedTaskTitles = new Set(
          processedData.stories.flatMap((story: APIProcessedStory) => 
            story.tasks.map((task: APIProcessedTask) => task.title.toLowerCase().trim())
          )
        )
