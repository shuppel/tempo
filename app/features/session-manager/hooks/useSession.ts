...
export const useSession = ({
  ...
  // Timer state
  const [activeTimeBox, setActiveTimeBox] = useState<{
    storyId: string;
    timeBoxIndex: number;
  } | null>(null);
  ...
  const updateSession = useCallback(
    async (updatedSession: Session): Promise<void> => {
      try {
  ...
  const calculateStoryProgress = (story: StoryBlock): number => {
    ...
    const completedTimeBoxes = workTimeBoxes.filter(
  ...
  const undoCompleteTimeBox = useCallback(
    (storyId: string, timeBoxIndex: number) => {
      if (!session) return;
      ...
      // Get the timebox
      const timeBox =
        updatedSession.storyBlocks[storyIndex].timeBoxes[timeBoxIndex];
  ...
  // Complete a timebox
  const completeTimeBox = useCallback(
    (storyId: string, timeBoxIndex: number) => {
      if (!session) return;
      ...
      const tasks =
        updatedSession.storyBlocks[storyIndex].timeBoxes[timeBoxIndex].tasks;
      if (tasks && tasks.length > 0) {
        tasks.forEach((task) => {
      ...
      if (timeBox.startTime) {
        ...
        const rawActualDuration = Math.round(
        ...
        console.log(
          `TimeBox completed - Type: ${timeBox.type}, ID: ${storyId}-${timeBoxIndex}`,
        );
        ...
        console.log(
          `  Time saved: ${timeBox.duration - timeBox.actualDuration}min`,
        );
        ...
        if (timeBox.type === "work") {
          ...
          timeBox.actualDuration = Math.max(
            1,
            Math.floor(timeBox.duration * 0.8),
          );
      ...
      if (
        activeTimeBox?.storyId === storyId &&
        activeTimeBox?.timeBoxIndex === timeBoxIndex
      ) {
      ...
      if (nextTimeBox) {
        ...
        if (nextStoryIndex !== -1) {
          const nextBoxType =
            updatedSession.storyBlocks[nextStoryIndex].timeBoxes[
              nextTimeBox.timeBoxIndex
            ].type;
          const boxTypeLabel =
            nextBoxType === "work"
              ? "focus session"
  ...
  const startTimeBox = useCallback(
    (storyId: string, timeBoxIndex: number, duration: number) => {
      if (!session) return;
  ...
  const handleTaskClick = useCallback(
    (
      ...
      if (!session || !storyId) {
        console.error(
          "Cannot toggle task status: session or storyId is undefined",
        );
        return;
      }

      console.log(
        "TASK UPDATE - Checking if task status changed in props:",
        task.status,
      );
      ...
      if (!timeBox || !timeBox.tasks) {
        console.error(
          `TimeBox or tasks array not found at index ${timeBoxIndex}`,
        );
        return;
      }
      ...
      const allTasksCompleted = timeBox.tasks.every(
        (t) => t.status === "completed",
      );

      // Auto-complete timebox if all tasks are completed
      if (allTasksCompleted) {
        updatedSession.storyBlocks[storyIndex].timeBoxes[timeBoxIndex].status =
          "completed";
        ...
        // Update story progress
        updatedSession.storyBlocks[storyIndex].progress =
          calculateStoryProgress(updatedSession.storyBlocks[storyIndex]);
      }
      ...
      storageService.updateTaskStatus(
        ...
        (task.status === "completed" ? "completed" : "todo") as
          | "todo"
          | "completed",
      );
    },
  ...
  const pauseTimer = useCallback(() => {
    if (!isTimerRunning || !session || !activeTimeBox || timeRemaining === null)
      return;
  ...
  const isCurrentTimeBox = useCallback(
    (timeBox: TimeBox) => {
  ...
  useEffect(() => {
    ...
    const timeBox =
      updatedSession.storyBlocks[storyIndex].timeBoxes[
        ...
        if (loadedSession) {
          ...
          if (timerState && timerState.activeTimeBox) {
            ...
            if (inProgressTimeBox) {
              ...
              if (storyIndex !== -1) {
                const timeBox = loadedSession.storyBlocks[storyIndex].timeBoxes[
                  inProgressTimeBox.timeBoxIndex
                ] as TimeBox;
                setTimeRemaining(
                  (timeBox as TimeBox & { remainingTime?: number })
                    .remainingTime ?? timeBox.duration * 60,
                );
              }
            }
          }
