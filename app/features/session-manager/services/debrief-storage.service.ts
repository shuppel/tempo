...
export class DebriefStorageService {
  ...
  getDebriefData(sessionDate: string): SessionDebriefData | null {
    try {
      ...
      return (
        allDebriefs.find((debrief) => debrief.sessionDate === sessionDate) ||
        null
      );
    } catch (error) {
...
export function useDebriefStorage() {
  ...
  const [debriefs, setDebriefs] = useLocalStorage<SessionDebriefData[]>(
    "session-debriefs",
    [],
  );
  ...
  const saveDebrief = async (data: SessionDebriefData) => {
    ...
    try {
      ...
      const existingIndex = currentDebriefs.findIndex(
        (d: SessionDebriefData) => d.sessionDate === data.sessionDate,
      );
  ...
  const getDebrief = (sessionDate: string): SessionDebriefData | undefined => {
    return debriefs?.find(
      (d: SessionDebriefData) => d.sessionDate === sessionDate,
    );
  };
