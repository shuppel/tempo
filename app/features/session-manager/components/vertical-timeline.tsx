...
export const VerticalTimeline = ({
  ...
  const [confirmTaskComplete, setConfirmTaskComplete] = useState<{
    storyId: string;
    timeBoxIndex: number;
    taskIndex: number;
    task: TimeBoxTask;
  ...
  const sessionDate =
    activeStoryId?.split("-")[0] || new Date().toISOString().split("T")[0];
  ...
  const safeCompleteTask = React.useCallback(
    (
      ...
      try {
        ...
        console.log(
          "Calling onTaskClick with status:",
          newStatus,
          "for task:",
          task.title,
        );
  ...
  const calculateAccumulatedDuration = (
    targetStoryIndex: number,
    targetTimeBoxIndex: number,
  ) => {
    let totalMinutes = 0;
    ...
    for (let si = 0; si <= targetStoryIndex; si++) {
      ...
      for (let bi = 0; bi <= boxLimit; bi++) {
        ...
        totalMinutes +=
          box.actualDuration !== undefined ? box.actualDuration : box.duration;
      }
    }
  ...
  const calculateTimeEstimates = (storyIndex: number, timeBoxIndex: number) => {
    ...
    try {
      ...
      const accumulatedMinutes = calculateAccumulatedDuration(
        storyIndex,
        timeBoxIndex,
      );
      ...
      estimatedStart.setMinutes(
        estimatedStart.getMinutes() + accumulatedMinutes,
      );
      ...
      if (currentTimeBox) {
        ...
        estimatedEnd.setMinutes(
          estimatedStart.getMinutes() + currentTimeBox.duration,
        );
        ...
        if (currentTimeBox.status === "completed" && currentTimeBox.startTime) {
          ...
          if (currentTimeBox.actualDuration !== undefined) {
            ...
            actualEndDate.setMinutes(
              actualStartDate.getMinutes() + currentTimeBox.actualDuration,
            );
      ...
      const isAdjusted = storyBlocks.some((story, sIdx) => {
        ...
        return story.timeBoxes.some((box, bIdx) => {
          ...
          return (
            box.actualDuration !== undefined &&
            box.actualDuration !== box.duration
          );
        });
      });
  ...
  const calculateSessionMetrics = React.useCallback(() => {
    ...
    storyBlocks.forEach((story) => {
      ...
      const allTimeboxesCompleted = story.timeBoxes.every(
        (tb) => tb.type !== "work" || tb.status === "completed",
      );
      if (
        allTimeboxesCompleted &&
        story.timeBoxes.some((tb) => tb.type === "work")
      ) {
        completedTasks++;
      }
      ...
      story.timeBoxes.forEach((timeBox) => {
        ...
        if (
          timeBox.status === "completed" &&
          timeBox.actualDuration !== undefined
        ) {
          if (timeBox.type === "work") {
            ...
            longestFocusStretch = Math.max(
              longestFocusStretch,
              currentFocusStretch,
            );
          } else if (
            timeBox.type === "short-break" ||
            timeBox.type === "long-break"
          ) {
            breakCount++;
    ...
    const averageBreakDuration =
      breakCount > 0 ? Math.round(totalBreakTime / breakCount) : 0;
    const focusConsistency =
      totalFocusSessions > 0
        ? Math.min(
            10,
            Math.round((completedFocusSessions / totalFocusSessions) * 10),
          )
        : 0;
    ...
    const averageTaskCompletionTime =
      completedTasks > 0
        ? Math.round(totalTaskCompletionTime / completedTasks)
        : 0;
      ...
      <div className="vertical-timeline-container">
        ...
        {!hasActiveTimebox && nextAction && !readyToStartPopupOpen && (
          <div className="fixed bottom-8 right-8 z-50">
            <Tooltip>
                ...
                <Button
                  ...
                  onClick={() => {
                    ...
                    if (nextAction && onStartTimeBox) {
                      const timeBox = storyBlocks.find(
                        (story) =>
                          story.id === nextAction.storyId ||
                          `story-${storyBlocks.indexOf(story)}` ===
                            nextAction.storyId,
              ...
              <TooltipContent
                side="left"
                className="z-[9999] bg-white dark:bg-gray-900 shadow-lg px-3 py-1.5 rounded-md border border-gray-200 dark:border-gray-800 text-sm"
              >
                <p>Start next task</p>
              </TooltipContent>
            </Tooltip>
          ...
          <AlertDialogContent className="max-w-md">
            <AlertDialogHeader>
              ...
              <AlertDialogDescription className="text-base text-gray-600 dark:text-gray-400">
                Your next task is ready and waiting for you. Let&apos;s go there
                now!
              </AlertDialogDescription>
            </AlertDialogHeader>
        ...
        >
          <AlertDialogContent onClick={(e) => e.stopPropagation()}>
            <AlertDialogHeader>
              ...
              <AlertDialogDescription>
                Are you sure you want to mark this timebox as complete? This
                will move you to the next step in your session.
              </AlertDialogDescription>
            </AlertDialogHeader>
            <AlertDialogFooter>
              <AlertDialogCancel
              ...
              <AlertDialogAction
                ...
                onClick={(e) => {
                  ...
                  if (confirmComplete && onCompleteTimeBox) {
                    try {
                      onCompleteTimeBox(
                        confirmComplete.storyId,
                        confirmComplete.timeBoxIndex,
                      );
                      console.log(
                        "Timebox completed:",
                        confirmComplete.storyId,
              ...
              <AlertDialogAction
                ...
                onClick={(e) => {
                  ...
                  if (confirmTaskComplete) {
                    console.log(
                      "Confirmed task completion for:",
                      confirmTaskComplete.task.title,
                    );
        ...
        <div className="mb-8 p-5 bg-gradient-to-r from-indigo-50 to-violet-50 dark:from-indigo-950/40 dark:to-violet-950/40 rounded-xl border border-indigo-100 dark:border-indigo-800/30 shadow-sm">
          <div className="flex items-center justify-between mb-4">
            <div>
              <h3 className="text-xl font-semibold bg-clip-text text-transparent bg-gradient-to-r from-indigo-700 to-purple-700 dark:from-indigo-400 dark:to-purple-400">
                Session Progress
              </h3>
            </div>
            <Badge
              variant="outline"
              className="px-3 py-1 border-indigo-200 dark:border-indigo-700/60 bg-white/80 dark:bg-gray-900/80 backdrop-blur-sm text-indigo-700 dark:text-indigo-300"
            >
            ...
            <div className="mt-2 flex justify-between text-xs px-1">
              ...
              <span
                className={cn(
                  ...
                  completedPercentage >= 12.5 &&
                    completedPercentage < 37.5 &&
                    "text-sm font-bold",
                )}
              >
                25%
          ...
          <div className="space-y-10">
            {storyBlocks.map((story, storyIndex) => {
              ...
              const totalTasks = story.timeBoxes.reduce(
                (sum, box) => sum + (box.tasks?.length || 0),
                0,
              );
              const completedTasks = story.timeBoxes.reduce(
                (sum, box) =>
                  sum +
                  (box.tasks?.filter((t) => t.status === "completed").length ||
                    0),
                0,
              );
              const storyProgress =
                ...
                <div key={storyId} className="relative">
                  ...
                  <div className="flex items-center mb-5">
                    <div
                      className={cn(
                        ...
                        isActiveStory &&
                          "ring-4 ring-indigo-200 dark:ring-indigo-700 ring-opacity-50",
                      )}
                    >
                  ...
                  <div className="space-y-4 ml-8 pl-14 relative">
                    {story.timeBoxes.map((timeBox, timeBoxIndex) => {
                      ...
                      const config =
                        timeboxTypeConfig[
                          timeBox.type as keyof typeof timeboxTypeConfig
                        ] || timeboxTypeConfig.work;
                      ...
                      const completedBoxTasks =
                        timeBox.tasks?.filter((t) => t.status === "completed")
                          .length || 0;
                      const boxProgress =
                        totalBoxTasks > 0
                          ? Math.round(
                              (completedBoxTasks / totalBoxTasks) * 100,
                            )
                          : isCompleted
                            ? 100
                            : isInProgress
                        ...
                        >
                              ...
                              className={cn(
                                ...
                                isCompleted
                                  ...
                                  : isActive
                                    ...
                                    : nextAction &&
                                        nextAction.storyId === storyId &&
                                        nextAction.timeBoxIndex === timeBoxIndex
                          ...
                          <div
                            className={cn(
                              ...
                              isActive
                                ? "bg-indigo-50 dark:bg-indigo-950/40 shadow-md border-indigo-400 dark:border-indigo-600"
                                : isInProgress
                                  ? statusColors.bg +
                                    " shadow-md " +
                                    statusColors.border
                              ...
                              nextAction &&
                                nextAction.storyId === storyId &&
                                nextAction.timeBoxIndex === timeBoxIndex &&
                                "next-action-card cursor-pointer",
                          ...
                          >
                            ...
                            <div className="flex items-center justify-between mb-2 relative">
                              <div className="flex items-center gap-2">
                                ...
                                {isActive && (
                                  <Badge className="bg-indigo-500 text-xs dark:bg-indigo-700/70 dark:text-indigo-100">
                                    Active
                                  </Badge>
                                )}
                                {isCompleted && (
                                  <Badge className="bg-green-500 text-xs dark:bg-green-700/70 dark:text-green-100">
                                    Completed
                                  </Badge>
                                )}
                                {isInProgress && !isActive && (
                                  <Badge className="bg-blue-500 text-xs dark:bg-blue-700/70 dark:text-blue-100">
                                    In Progress
                                  </Badge>
                                )}
                                {nextAction &&
                                    ...
                                    >
                                      <span className="inline-flex items-center justify-center mr-1.5 text-xs">
                                        <svg
                                          ...
                                          className="animate-pulse animate-play-icon"
                                        >
                                          <path
                                        ...
                                        <div className="flex flex-col gap-1">
                                          <div className="flex items-center justify-between">
                                            <span className="font-medium text-sm">
                                              Duration: {timeBox.actualDuration}{" "}
                                              minutes
                                            </span>
                                          ...
                                          {timeEstimates.actualStart &&
                                            timeEstimates.actualEnd && (
                              ...
                              timeBox.tasks.length > 0 && (
                                ...
                                >
                                  {timeBox.tasks.map((task, taskIndex) => {
                                    ...
                                    const isNextTask =
                                      !isTaskCompleted && timeBox.tasks
                                        ? taskIndex ===
                                          (timeBox.tasks.findIndex(
                                      ...
                                      >
                                        <div
                                          className={cn(
                                            ...
                                            isTaskCompleted
                                              ? statusColorConfig.completed.bg +
                                                  " " +
                                                  statusColorConfig.completed
                                                    .border
                                      ...
                                      <Button
                                        ...
                                        onClick={(e) => {
                                          ...
                                          if (
                                            ...
                                            const firstIncompleteTaskIndex =
                                              timeBox.tasks.findIndex(
                                                (t) => t.status !== "completed",
                                              );
                                            if (
            ...
            <div className="ml-7 relative">
              ...
              <div className="rounded-xl p-4 border-l-4 mb-2 transition-all overflow-hidden relative bg-white hover:bg-gray-50 dark:bg-gray-950 dark:hover:bg-gray-900 border-rose-200 dark:border-rose-800 hover:scale-[1.02] hover:shadow-md transform-gpu transition-transform duration-200 ml-8 pl-6">
                ...
                <div className="text-sm text-gray-600 dark:text-gray-400 mt-1 mb-3">
                  <p>
                    Reflect on your completed session and capture any important
                    insights.
                  </p>
                </div>
                          ...
                          onClick={() => {
                            ...
                            if (
                              ...
                              if (existingDebrief) {
                                ...
                                toast({
                                  ...
                                  description:
                                    "You have already completed the debrief for this session.",
                                  variant: "default",
                                });
                                return;
                              }
                            ...
                            if (
                              ...
                              toast({
                                ...
                                description:
                                  "You need to complete all tasks before starting the debrief.",
                                variant: "destructive",
                              });
                            }
                          }}
        ...
        <SessionDebriefModal
          ...
          onSave={async (data) => {
            ...
            toast({
              ...
              description:
                "Your session reflection has been saved. Redirecting to home page.",
              duration: 3000,
            });
        ...
        <style jsx>{`
          ...
          .dark .progress-gradient {
            background: linear-gradient(
              to right,
              #db2777,
              /* Darker pink for dark mode */ #ea580c,
              /* Darker orange for dark mode */ #8b5cf6,
              /* Darker purple for dark mode */ #22c55e
          ...
          .timebox-progress-gradient {
            background: linear-gradient(
              to right,
              #a78bfa,
              /* Light purple */ #34d399 /* Emerald green */
            );
          ...
          .dark .timebox-progress-gradient {
            background: linear-gradient(
              to right,
              #8b5cf6,
              /* Darker purple for dark mode */ #10b981
                /* Darker emerald for dark mode */
            );
