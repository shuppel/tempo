...
export const TimeboxView = ({
  ...
  useEffect(() => {
    const hasSeenButtonGuidance = localStorage.getItem(
      "hasSeenStartButtonGuidance",
    );
  ...
  const isTimeBoxActiveById = useCallback(
    (storyId: string, timeBoxIndex: number): boolean => {
  ...
  const isNextAction = (storyId: string, timeBoxIndex: number) => {
    ...
    return (
      ...
      !hasActiveTimebox
    ); // Don't highlight next action if any timebox is active
  };
  ...
  const totalDuration = storyBlocks.reduce(
    (total, story) =>
      total + story.timeBoxes.reduce((sum, box) => sum + box.duration, 0),
    0,
  );
  ...
  const totalWorkBoxes = storyBlocks.flatMap((s) =>
    s.timeBoxes.filter((t) => t.type === "work"),
  ).length;
  const completedWorkBoxes = storyBlocks.flatMap((s) =>
    s.timeBoxes.filter((t) => t.type === "work" && t.status === "completed"),
  ).length;
            ...
            <div>
              ...
              <div className="flex flex-wrap gap-x-4 gap-y-1 mt-1 text-sm text-muted-foreground">
                ...
                <div className="flex items-center gap-1">
                  ...
                  <span>
                    Breaks: {Math.floor(breakDuration / 60)}h{" "}
                    {breakDuration % 60}m
                  </span>
                </div>
              </div>
              ...
              <div key={storyId} className="space-y-4">
                ...
                <div className="relative pl-6 border-l-2 border-gray-200 dark:border-gray-700 space-y-5 ml-2">
                  {story.timeBoxes.map((timeBox, timeBoxIndex) => {
                    ...
                    const isCurrentFirstTodo = isFirstTodoTimeBox(
                      storyId,
                      timeBoxIndex,
                    );
                    const shouldHighlightNextAction = isNextAction(
                      storyId,
                      timeBoxIndex,
                    );
                          ...
                          <div className="flex justify-between items-center mb-2">
                            <div className="flex items-center gap-2">
                              ...
                              {isCompleted && (
                                <Badge
                                  variant="secondary"
                                  className="bg-green-50 border-green-200 text-green-700 dark:bg-green-950/30 dark:border-green-900 dark:text-green-400"
                                >
                                  Completed
                                </Badge>
                              )}
                              {isActive && (
                                <Badge className="bg-blue-50 border-blue-200 text-blue-700 dark:bg-blue-950/30 dark:border-blue-900 dark:text-blue-400">
                                  In Progress
                                </Badge>
                              )}
                              {shouldHighlightNextAction && (
                                <Badge className="bg-purple-50 border-purple-300 text-purple-700 dark:bg-purple-950/30 dark:border-purple-900 dark:text-purple-400">
                                  Next Action
                                </Badge>
                              )}
                            </div>
                                  ...
                                  <TooltipTrigger asChild>
                                    <Button
                                      ...
                                      onClick={() => {
                                        if (
                                          window.confirm(
                                            "Are you sure you want to go back to this task?",
                                          )
                                        ) {
                                          onUndoCompleteTimeBox(
                                            storyId,
                                ...
                                shouldHighlightNextAction ? (
                                  <div className="relative">
                                      ...
                                      isCurrentFirstTodo && (
                                        ...
                                        >
                                          ...
                                          <p className="text-xs mt-1">
                                            Click this button to start your{" "}
                                            {isWork
                                              ? "first focus session"
                                              : "break"}
                                            .
                                    ...
                                    <motion.div
                                      ...
                                      transition={{
                                        repeat: storyBlocks.some((story) =>
                                          story.timeBoxes.some((box) =>
                                            isCurrentTimeBox(box),
                                          ),
                                        )
                                          ? 0
                                          : Infinity,
