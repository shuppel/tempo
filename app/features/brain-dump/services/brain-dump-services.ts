...
const modifyStoriesForRetry = (
  ...
  error:
    | { details?: { block?: string; preventiveModification?: boolean } }
    | unknown,
): ProcessedStory[] => {
  // Create a deep copy to avoid mutating the original data
  const modifiedStories = JSON.parse(
  ...
  console.log(
    "Modifying stories for retry. Error details:",
  ...
  modifiedStories.forEach((story) => {
    console.log(
      `Processing story "${story.title}" with ${story.tasks.length} tasks`,
    );
    ...
    const isProblematicBlock =
      problematicBlock && story.title === problematicBlock;
    ...
    story.tasks.forEach((task, taskIndex) => {
      ...
      const wouldExceedLimit =
        cumulativeWorkTime + task.duration >
      ...
      if (task.duration > maxTaskDuration || wouldExceedLimit) {
        console.log(
          `Splitting task "${task.title}" (${task.duration} minutes) into smaller parts`,
        );
        ...
        const numParts = Math.max(
          2,
          Math.ceil(task.duration / effectiveMaxTaskDuration),
        );
    ...
    if (isProblematicBlock) {
      ...
      for (let i = 0; i < updatedTasks.length; i++) {
            ...
            task.suggestedBreaks = [
              {
                ...
                duration: DURATION_RULES.LONG_BREAK,
                reason:
                  "Required break to prevent excessive consecutive work time",
              },
            ];
          }
        ...
        if (
          task.suggestedBreaks &&
          task.suggestedBreaks.some((b) => b.after === task.duration)
        ) {
          runningWorkTime = 0;
        }
      }
...
function recalculateStoryDuration(story: ProcessedStory): void {
  ...
  const totalWorkTime = story.tasks.reduce(
  ...
  const totalBreakTime = story.tasks.reduce(
    (sum, task) =>
      sum +
      (task.suggestedBreaks?.reduce((bSum, b) => bSum + b.duration, 0) || 0),
    0,
  );
...
const createSession = async (
  ...
  console.log(
    `Created mapping for ${titleToStoryMap.size} potential story titles`,
  );

  // Only validate that the totalDuration is a valid multiple of BLOCK_SIZE and above MIN_DURATION
  const totalDuration = currentStories.reduce(
  ...
  while (retryCount < maxRetries) {
    try {
      console.log(
        `Attempting to create session (attempt ${retryCount + 1}/${maxRetries})`,
      );
      ...
      const request = {
        ...
        storyMapping: Array.from(titleToStoryMap.keys()).map((title) => ({
          possibleTitle: title,
          originalTitle:
            titleToStoryMap.get(title)?.originalTitle ||
            titleToStoryMap.get(title)?.title,
        })),
      };
      ...
      if (typeof data.totalDuration !== "number" || data.totalDuration <= 0) {
        ...
        console.warn(
          "Session response missing valid totalDuration, calculating from story blocks",
        );
        ...
        if (calculatedTotalDuration <= 0) {
          throw new Error(
            "Cannot calculate valid session duration from story blocks",
            {
        ...
        console.log(
          `Set totalDuration to calculated value: ${calculatedTotalDuration} minutes`,
        );
      }
      ...
      try {
        ...
        const sessionToSave = {
          ...
          storyBlocks: (data.storyBlocks as StoryBlock[]).map(
            (block: StoryBlock, index: number) => ({
              ...block,
              id: block.id || `story-${index}-${Date.now()}`,
              progress: 0,
              timeBoxes: (block.timeBoxes || []).map((timeBox: TimeBox) => ({
                ...(typeof timeBox === "object" && timeBox !== null
                  ? timeBox
                  : {}),
                status:
                  typeof timeBox === "object" &&
                  timeBox !== null &&
                  "status" in timeBox
                    ? (timeBox as { status?: string }).status || "todo"
                    : "todo",
                // Ensure task fields are correctly structured
                tasks: (typeof timeBox === "object" &&
                timeBox !== null &&
                "tasks" in timeBox &&
                Array.isArray((timeBox as TimeBox).tasks)
                  ...
                  status:
                    ...
                    task !== null &&
                    "status" in task
                      ? (task as { status?: string }).status || "todo"
                      : "todo",
                })),
              })),
            }),
        ...
        console.log(
          `Session saved using SessionStorageService for date: ${today}`,
        );
        ...
        if (!savedSession) {
          console.error(
            "Failed to verify saved session - not found in session storage",
          );
      ...
      await new Promise((resolve) =>
        setTimeout(resolve, shouldRetry ? 2000 : 1000),
      );
    }
  }
