...
export default function SessionsPage() {
  ...
  const handleDeleteSession = async () => {
    if (!sessionToDelete || confirmationText !== DELETE_CONFIRMATION_TEXT)
      return;

    try {
      await storageService.deleteSession(sessionToDelete);
      setSessions(
        sessions.filter((session) => session.date !== sessionToDelete),
      );
      toast({
    ...
    try {
      ...
      if (success) {
        ...
        toast({
          ...
          description:
            "The session has been successfully unarchived and moved to active sessions.",
        });
      } else {
  ...
  const sortedArchivedSessions = [...archivedSessions].sort(
    (a, b) => parseISO(b.date).getTime() - parseISO(a.date).getTime(),
  );
  ...
  const MemoizedClockIcon = useMemo(() => <Clock className="h-4 w-4" />, []);
      ...
      >
        <main className="flex-1 container mx-auto p-4 md:p-8 max-w-4xl">
                        ...
                        <Progress
                          value={
                            session.storyBlocks.reduce(
                              (sum, block) => sum + (block.progress || 0),
                              0,
                  ...
                  <CardContent className="py-8">
                    <div className="text-center text-muted-foreground">
                      ...
                      <p className="text-sm mt-1">
                        Plan your first session to get started
                      </p>
              ...
              <div className="space-y-4">
                ...
                <div className="grid gap-2">
                  {sortedArchivedSessions.map((session) => (
                    <Card
                      key={session.date}
                      className="transition-colors hover:bg-muted/50 bg-muted/20"
                    >
                            ...
                            <DropdownMenu>
                              ...
                              >
                                <DropdownMenuItem
                                  ...
                                  onClick={() =>
                                    handleUnarchiveSession(session.date)
                                  }
                                >
          ...
          <AlertDialogContent>
            <AlertDialogHeader>
              <AlertDialogTitle>
                Delete {sessionToDelete ? "Session" : ""}
              </AlertDialogTitle>
              <AlertDialogDescription>
                This action cannot be undone. This will permanently delete the
                session
                {sessionToDelete &&
                  ` for ${safeFormatDate(sessionToDelete, "MMMM d, yyyy")}`}
                .
              </AlertDialogDescription>
            </AlertDialogHeader>
            <div className="my-4">
