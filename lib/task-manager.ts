...
function calculateSplitDurations(totalDuration: number): {
  ...
  while (remaining > DURATION_RULES.SHORT_BREAK) {
    if (
      remaining <=
      DURATION_RULES.FINAL_SESSION.max + DURATION_RULES.SHORT_BREAK
    ) {
      ...
      const middlePart = Math.min(
        45,
        Math.floor((remaining - DURATION_RULES.LONG_BREAK) * 0.5),
      );
...
async function analyzeAndGroupTasks(
  ...
  sortedTasks.forEach((task) => {
    ...
    const storyName =
      task.title.split(":")[0] || task.title.split(" - ")[0] || task.title;
    if (!groups[storyName]) {
      groups[storyName] = [];
    }
...
export async function createTimeBoxes(tasks: Task[]): Promise<SessionPlan> {
  ...
  const storyBlockTotal = storyBlocks.reduce(
    (sum, block) => sum + block.totalDuration,
    0,
  );
  ...
  return {
    ...
    endTime: new Date(
      now.getTime() + totalSessionDuration * 60000,
    ).toISOString(),
    frogMetrics: {
...
export function organizeTasks(tasks: Task[]): Task[] {
  return tasks.sort((a, b) => {
    ...
    if (
      aDiffValue >= DIFFICULTY_WEIGHTS.PHYSICAL &&
      bDiffValue < DIFFICULTY_WEIGHTS.PHYSICAL
    )
      return -1;
    if (
      bDiffValue >= DIFFICULTY_WEIGHTS.PHYSICAL &&
