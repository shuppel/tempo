...
export const sessionStorage = {
  ...
  async saveSession(date: string, session: StoredSession): Promise<void> {
    try {
      ...
      if (isReplicacheAvailable() && replicacheClient) {
        ...
        await replicacheClient.mutate.saveSession({
          date,
  ...
  async updateTimeBoxStatus(
    ...
    try {
      if (isReplicacheAvailable() && replicacheClient) {
        ...
        return await replicacheClient.mutate.updateTimeBoxStatus({
          date,
          storyId,
          timeBoxIndex,
          status,
        ...
        let updated = false;
        const updatedStoryBlocks = sessionData.storyBlocks.map(
          (story: StoryBlock) => {
            if (story.id === storyId && story.timeBoxes[timeBoxIndex]) {
              ...
              if (
                status === "completed" &&
                updatedTimeBoxes[timeBoxIndex].tasks
              ) {
                updatedTimeBoxes[timeBoxIndex].tasks = updatedTimeBoxes[
                  timeBoxIndex
                ].tasks.map((task: TimeBoxTask) => ({
                  ...task,
              ...
              const workBoxes = updatedTimeBoxes.filter(
                (box: TimeBox) => box.type === "work",
              );
              const completedWorkBoxes = workBoxes.filter(
                (box: TimeBox) => box.status === "completed",
              );
              const progress =
                workBoxes.length > 0
                  ? Math.round(
                      (completedWorkBoxes.length / workBoxes.length) * 100,
                    )
                  : 0;
        ...
        if (updated) {
          ...
          const anyInProgress = allWorkBoxes.some(
            (box: TimeBox) => box.status === "in-progress",
          );
          const anyCompleted = allWorkBoxes.some(
            (box: TimeBox) => box.status === "completed",
          );
  ...
  async updateSessionProgress(date: string): Promise<boolean> {
    ...
    const updatedStoryBlocks = session.storyBlocks.map((story: StoryBlock) => {
      const totalWorkBoxes = story.timeBoxes.filter(
        (box: TimeBox) => box.type === "work",
      ).length;
  ...
  isValidSession(data: unknown): data is StoredSession {
    ...
    // Use a type assertion for property checks
    const obj = data as Record<string, unknown>;
    // Special case: If this is a timer-only state object without full session data
    if (
      "activeTimeBox" in obj &&
      "timeRemaining" in obj &&
      "isTimerRunning" in obj
    ) {
      if (!("storyBlocks" in obj)) {
        // This is just timer state without the full session, which is valid for our timer persistence use case
        (obj.storyBlocks as unknown) = [];
        (obj.totalSessions as unknown) =
          (obj.totalSessions as number | undefined) || 1;
        (obj.startTime as unknown) =
          (obj.startTime as string | undefined) || new Date().toISOString();
        (obj.endTime as unknown) =
          (obj.endTime as string | undefined) || new Date().toISOString();
        return true;
      }
    }
    // Check for minimal required properties
    if (!("storyBlocks" in obj) || !Array.isArray(obj.storyBlocks)) {
      console.error("Invalid session: storyBlocks is not an array", data);
      return false;
    }
    // Set defaults for other properties if they're missing
    if (typeof obj.totalSessions !== "number") {
      console.warn("Session missing totalSessions, using default", data);
      (obj.totalSessions as unknown) = 1;
    }
    if (typeof obj.startTime !== "string") {
      console.warn("Session missing startTime, using default", data);
      (obj.startTime as unknown) = new Date().toISOString();
    }
    if (typeof obj.endTime !== "string") {
      console.warn("Session missing endTime, using default", data);
      (obj.endTime as unknown) = new Date().toISOString();
    }
    return true;
  },
  ...
  normalizeSession(session: StoredSession): StoredSession {
    ...
    const normalizedStoryBlocks = storyBlocks.map((block) => {
      ...
      if (progress === undefined || progress === null) {
        const totalWorkBoxes = normalizedTimeBoxes.filter(
          (box) => box.type === "work",
        ).length;
    ...
    return {
      ...
      totalSessions:
        typeof session.totalSessions === "number" ? session.totalSessions : 1,
      startTime: session.startTime || new Date().toISOString(),
      endTime: session.endTime || new Date().toISOString(),
      lastUpdated:
        session.lastUpdated || session.startTime || new Date().toISOString(),
